...
export function StatsDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    weakestPatterns: [] as { pattern: string; avgConfidence: number; count: number }[],
    recentActivity: [] as { date: string; count: number }[],
  })

  useEffect(() => {
    const stored: Problem[] = JSON.parse(localStorage.getItem("leetcode-problems") || "[]")
    setProblems(stored)
  ...
  const calculateStats = (problems: Problem[]) => {
    ...
    const totalTimeSpent = problems.reduce((sum, p) => sum + p.attempts.reduce((s, a) => s + a.timeSpent, 0), 0)
    const averageConfidence =
      problems.length > 0 ? problems.reduce((sum, p) => sum + p.confidence, 0) / problems.length : 0
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Distribution of problems you've solved</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {Object.entries(stats.byDifficulty).map(([difficulty, count]) => {
              ...
              const color =
                difficulty === "Easy" ? "bg-success" : difficulty === "Medium" ? "bg-yellow-500" : "bg-destructive"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Which patterns you've practiced most</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {Object.entries(stats.byPattern)
              .sort(([, a], [, b]) => b - a)
              .map(([pattern, count]) => (
                <div key={pattern} className="p-3 border border-border rounded-lg">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Patterns with lowest confidence - prioritize reviewing these</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {stats.weakestPatterns.map((item) => (
              <div key={item.pattern} className="flex items-center justify-between p-3 border border-border rounded-lg">
                <div>
                  <p className="font-medium">{item.pattern}</p>
                  <p className="text-sm text-muted-foreground">{item.count} problems</p>
                </div>
                <div className="text-right">
                  <p className={`text-lg font-bold ${getConfidenceColor(item.avgConfidence)}`}>
            ...
            {stats.weakestPatterns.length === 0 && (
              <p className="text-center text-muted-foreground py-8">No data yet. Keep solving problems!</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Your problem-solving activity over the last 30 days</CardDescription>
        </CardHeader>
        <CardContent>
          {stats.recentActivity.length > 0 ? (
                ...
                <div key={activity.date} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 bg-muted rounded-full h-6 relative overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.min((activity.count / 5) * 100, 100)}%` }}
                    />
                    <span className="absolute inset-0 flex items-center justify-center text-xs font-medium">
                      {activity.count} {activity.count === 1 ? "problem" : "problems"}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <p className="text-center text-muted-foreground py-8">No recent activity. Start solving problems!</p>
          )}
        </CardContent>
      </Card>
    </div>
